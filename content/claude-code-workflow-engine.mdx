...
*Part 2 of a series on using Claude Code as a production runtime. Originally published on [everyrow.io](https://everyrow.io/blog/claude-code-workflow-engine).*
...
Running it is what we covered in the [Part 1](/claude-code-kubernetes-cronjob/). You pass `"execute scan-and-classify skill"` as a prompt and it runs. Again, you don't have to think about deployments and flags and if you should use `deploy()` or `serve()` or anything, just CLI command.
...
**Layer 3: Bash `timeout 10800`.** After 3 hours, a second Claude wakes up to salvage partial results (see [Post 1](/claude-code-kubernetes-cronjob/)).
...
*We build [everyrow.io](https://everyrow.io) - semantic deduplication, entity resolution, and qualitative ranking for datasets. This pipeline is how we find the people who need these tools.*

---

*Next: [Marketing Pipeline Using Claude Code](/marketing-pipeline-using-claude-code/)*
