export const metadata = {
  title: "Simple Slack to Mattermost migration",
  slug: "simple-slack-to-mattermost-migration",
  description:
    "A short Python script that converts a Slack export into Mattermost's bulk-load format, written after Slack started deleting free-tier message history.",
  date: "2024-08-18",
  tags: ["IT","contribution"],
};

This is very short post mostly just to make it indexed by Google. With Slack deleting the content after a year on the Free plan, I decided to try to switch our household to Mattermost. Here is a simple [gist](https://gist.github.com/hnykda/9af89157d4588d978202da9bcaede278) that does that.
...
Run it as `python converter.py <folder-with-slack-export> output.jsonl`. It does not support attachments and a lot of bell and whistles. Just basic messages. Note that it is not idempotent as docs suggest. E.g. if it fails after you start loading e.g. users, you then have to not load the users again. 

You have to then:
1. zip it `zip -r data.zip output.jsonl`
2. `EnableLocalMode: true` in your mattermost server config (and restart)
