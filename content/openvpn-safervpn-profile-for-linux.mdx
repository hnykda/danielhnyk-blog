export const metadata = {
  title: "OpenVPN SaferVPN profile for Linux",
  slug: "openvpn-safervpn-profile-for-linux",
  date: "2017-06-04",
  tags: ["linux","contribution","IT"],
};

I stumbled upon a problem when I need to use VPN located in UK. There are many VPN providers, but only few of them provide OpenVPN profiles so you can use them with OpenVPN CLI on Linux.

I went with SaferVPN (they offer 24 hours trial withou credit card) and they fortunately provides them, although a bit hidden from everywhere. They can be found on [this page](https://www.safervpn.com/support/articles/214036025-SaferVPN-OpenVPN-configuration-files). For me, the UK one worked flawlessly. Credentials are just those you used for account creation (that means your full email and password).

E.g. for UK (UDP), this worked nice:
```
client
dev tun
proto udp
remote uk1.safervpn.com 1194 # - Your server IP and OpenVPN Port
resolv-retry infinite
nobind
tun-mtu 1500
tun-mtu-extra 32
mssfix 1450
persist-key
persist-tun
auth-user-pass
comp-lzo
verb 3
<ca>
-----BEGIN CERTIFICATE-----

ibQxtH8dV3ItFb3AZpHRurL/UmTR
-----END CERTIFICATE-----
</ca>
```
